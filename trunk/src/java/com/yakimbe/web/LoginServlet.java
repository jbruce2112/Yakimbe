    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (!strUserName.isEmpty() && !strPassword.isEmpty()) {
            try {
                ...
                returnedUser = loginUser(strUserName, strPassword, connection, request);

                String fbID = (String) request.getSession().getAttribute("hookWithFBID");

                if (request.getSession().getAttribute("hookWithFBAccount") != null &&
                    fbID != null && returnedUser != null) {

                    hookWithFacebook(returnedUser.getId(), fbID, connection);
                    
                    List<String> messages = new ArrayList<String>();
                    messages.add("Thanks - you can now use your Facebook account to log in to Yakimbe.");
                    messages.add("<a href='/'>Go Home</a>");
                    request.setAttribute("messages", messages);

                    request.getSession().removeAttribute("hookWithFBAccount");
                    request.getSession().removeAttribute("hookWithFBID");
                }
                ...
                if (returnedUser != null) {
                    ...
                    request.setAttribute("errors", listLoginErrors);
                }
...
*/
    ...
    protected static void hookWithFacebook(final int userID, final String fbID, final Connection conn) throws SQLException {
        PreparedStatement ps = null;
        final String query = "UPDATE user SET fb_id = ? WHERE user_id = ?";
    ...
    protected static User loginUser(final String userName, final String plainPw, final Connection conn,
                                    final HttpServletRequest req) throws SQLException, IllegalArgumentException {
        ...
        psUserLogin = conn.prepareCall("SELECT * FROM user u WHERE u.user_name = ?");
        ...
        if (rsUserLogin.next()) {
            ...
            String hashedPw = rsUserLogin.getString("pw");
            ...
            String fbID = rsUserLogin.getString("fb_id");
            ...
            boolean passwordsMatch = BCrypt.checkpw(plainPw, hashedPw);

            if (passwordsMatch) {
                if (isAccountVerified == 1 || fbID != null) {
                    ...
                    returnedUser.setFacebookID(fbID);

                    //user notifications
                    returnedUser.setNotifications(getNotifications(req, conn, uID));

                    return returnedUser;
                }
                else {
                    req.setAttribute("userName", uName);
                    req.setAttribute("userStatus", "accountUnverified");
                }
            }
        }
        return null;
    ...
    protected static Map<String,List> getNotifications(HttpServletRequest req, Connection conn, int userId) {
